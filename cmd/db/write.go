...
func Test2(logger *log.Logger) {
	...
	compressed, err := common.CompresStrToB64(logger, m1)
	if err != nil {
		logger.Fatalf("cant compress %s: %s", m1, err)
	}

	// Decode the string
	decoded, err := base64.StdEncoding.DecodeString(m1)
	if err != nil {
		logger.Fatal("Failed to decode base64 string")
	}
	...
	logger.Printf("%d rows affected", result.RowsAffected)
