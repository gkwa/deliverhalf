...
func getSnsMessageFromStr(str string) (types.Message, error) {
	...
	if err != nil {
		...
		if err != nil {
			return message, fmt.Errorf("can't unmarshal %s into types.Message: %s", str, err)
		}
...
func WriteToDb(db *gorm.DB, str string) {
...
func Test2() {
	db, err := ConnectToSQLiteDB("test.db")
	if err != nil {
	...
	db.AutoMigrate(&imds.IdentityBlob{})
